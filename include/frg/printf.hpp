			...
			switch(pos_args[i].type) {
				...
				default:
					if constexpr (std::is_same_v<Char, char>) {
						...
						std::ignore = agent(L"frigg: unhandled printf_arg_type ", 33);
						std::ignore = agent(L'0' + std::to_underlying(pos_args[i].type));
						std::ignore = agent(L'\0');
					}
					return frg::format_error::agent_error;
			}
		}
...
void do_printf_floats(S &sink, Char t, format_options opts,
	...
	case 'A':
		...
		if (szmod == printf_size_mod::longdouble_size) {
			_fmt_basics::print_float<S, long double, typename S::value_type>(
		...
		_fmt_basics::print_float<S, double, typename S::value_type>(
